...
export function initCatCursor() {
  ...
  catElement.innerHTML = `
    <svg viewBox="0 0 64 40" width="44" height="32">
      ...
      <!-- Cat Eye -->
      <circle cx="50" cy="12" r="1.5" fill="#030712" />
      <!-- Pink Nose -->
  ...
  // Hide default cursor across page, restore on clickables
  ...
  function addPawPrint(x, y, angle) {
    const now = Date.now();
    if (now - lastPawTime < 120) return; // limit paw density
    lastPawTime = now;
  ...
  function drawPawPrint(paw) {
    ...
    // Glowing Violet Paw Print SVG shape draw
    ...
    // Toe Dots
    const toes = [-4, -1.5, 1.5, 4];
    const toeY = [-3, -5, -5, -3];
    toes.forEach((tx, i) => {
      ctx.beginPath();
      ctx.arc(tx, toeY[i], 1.5, 0, Math.PI * 2);
      ctx.fill();
    });
  ...
  function loop() {
    ...
    // Calculate rotation angle when moving
    if (dist > 2) {
